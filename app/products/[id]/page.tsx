...
import {
    ...
    Heart, Share2, Loader2, ZoomIn, X, ChevronLeft, ChevronDown
} from "lucide-react"
...
import { fetchProductById, fetchProductReviews, submitReview, fetchProducts } from "@/lib/d1-client"
...
export default function ProductDetailPage() {
    ...
    const loadData = async () => {
        setLoading(true)
        try {
            // First: get the product so we have the real UUID
            const prod = await fetchProductById(id)
            setProduct(prod)

            if (prod) {
                // Use prod.id (UUID) — not the URL slug — for reviews
                const realId = prod.id || id
                ...
                setRelatedProducts((allProds as Product[]).filter(p => p.category === prod.category && p.id !== prod.id && p.isActive).slice(0, 4))
            }
            ...
            <div className="container mx-auto px-4 py-12 animate-pulse">
                <div className="grid lg:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-4">
                        <div className="h-8 bg-gray-200 rounded w-3/4" /><div className="h-4 bg-gray-200 rounded w-1/4" /><div className="h-10 bg-gray-200 rounded w-1/3" /><div className="h-24 bg-gray-200 rounded" />
                    </div>
                </div>
            </div>
        </div>
