...
export function ShopAIChat() {
  ...
  useEffect(() => {
    if (!hydrated) return
    try {
      // Only persist last 40 messages (no products data — too heavy)
      const toSave = messages.slice(-40).map((m) => ({
        ...m,
        products: undefined, // don't cache products (stale data risk)
      }))
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "ai",
          text: "GrabNext AI is offline right now. Check your connection and try again! 🙏",
          timestamp: new Date().toISOString(),
          products: [],
        },
      ])
    ...
    <>
      ...
      <div className="fixed bottom-5 right-5 z-[9999]">
        <button
          ...
          className="flex items-center gap-2.5 bg-white rounded-full shadow-xl border border-gray-100 px-4 py-2.5 hover:shadow-2xl hover:scale-105 transition-all duration-300 group"
        >
          {/* AI Icon — sky blue circle, sparkle icon, no extra background noise */}
          <div className="h-8 w-8 rounded-full bg-sky-500 flex items-center justify-center shrink-0 shadow-md group-hover:bg-sky-600 transition-colors">
            <Sparkles className="h-4 w-4 text-white" />
          </div>
          <span className="text-gray-700 font-semibold text-sm pr-1 whitespace-nowrap">Shop with AI</span>
        </button>
      </div>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="bg-sky-500 px-4 py-3 flex items-center justify-between shrink-0">
            ...
            {/* Close button (only here — no bottom X) */}
            <button
            ...
            {messages.map((msg, i) => (
              <div key={i} className="space-y-2">
                <div className={`flex items-end gap-2 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  {/* AI Avatar — just the icon, no background box */}
                  {msg.role === "ai" && (
                    <div className="shrink-0 mb-0.5 flex items-end">
                  ...
                  <div className="ml-7 space-y-2">
                    {msg.products.map((product) => {
                      ...
                      return (
                        ...
                        >
                          {/* Clickable top section → product page */}
                          <Link
                            href={productUrl}
                            onClick={() => setIsOpen(false)}
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            ...
            <p className="text-[10px] text-gray-400 text-center mt-1.5">
              <span className="text-sky-500 font-semibold">GrabNext AI</span> · Powered by Gemini
            </p>
          </div>
        </div>
      )}
