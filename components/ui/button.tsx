...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, children, ...props }, ref) => {
    ...
    // When asChild is true, we cannot inject the loading spinner as a sibling 
    // because Slot expects a single child. Loading state should be handled by the child
    // or by not using asChild when loading is needed.
    if (asChild) {
      return (
        <Comp
